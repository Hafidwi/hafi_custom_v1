frappe.ui.form.on('Employee Advance', {
    refresh: function(frm) {
        if (frm.doc.docstatus === 1 && frm.doc.paid_amount === 0) {
            frm.add_custom_button(__('Journal Entry'), function() {
                ...
                frappe.model.with_doctype('Journal Entry', function() {
                    ...
                    je.custom_type = 'Pay'; // Gunakan payment_type, bukan custom_type

                    // Baris 1: Hutang Kasbon (Debit)
                    let row1 = frappe.model.add_child(je, 'accounts');
                    row1.account = frm.doc.advance_account;
                    row1.party_type = 'Employee';
                    row1.party = frm.doc.employee;
                    row1.debit_in_account_currency = frm.doc.advance_amount;
                    row1.reference_type = 'Employee Advance';
